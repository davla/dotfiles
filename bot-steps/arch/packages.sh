    ...
    case "$HOST" in
        'personal')
            ...
            sudo -u "$USER_NAME" yay -S --needed alacritty asunder atril \
                ...
                firefox-beta-bin geany gnome-disk-utility gnome-keyring gufw \
                handbrake kid3 libreoffice-still mpv remmina seahorse \
                simple-scan soundconverter spotify telegram-desktop \
...
sudo -u "$USER_NAME" yay -S --needed asdf-vm autoconf automake bind cmake \
    cmatrix cowsay curl debugedit dkms docker-compose dos2unix eza fasd \
    ...
    otf-ipafont pacman-contrib passt pkgfile playerctl podman-docker python \
    python-pip rbw sheldon sl sudo thefuck ttf-baekmuk ttf-dejavu \
...
id "$USER_NAME" \
    ...
    | {
        ...
        usermod --add-subuids "$SUB_UID_START-$(( SUB_UID_START + 65536 ))" \
            "$USER_NAME"

        SUB_GID_START="$(( GROUP_ID + 99000 ))"
        usermod --add-subgids "$SUB_GID_START-$(( SUB_GID_START + 65536 ))" \
            "$USER_NAME"
    }
