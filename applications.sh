...
apt-get install aisleriot asunder atom baobab blueman brasero calibre \
    ...
    network-manager-gnome numix-theme parcellite pavucontrol quadrapassel \
...
apt-get install autoconf cowsay cups curl dkms dos2unix flashplayer-mozilla \
    ...
    browser-plugin-vlc p7zip python-requests-futures python3-lxml rar ruby \
    ruby-dev sbt scala sudo thunar-archive-plugin thunar-dropbox-plugin \
