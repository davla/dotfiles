...
apt-get install apt-transport-https autoconf automake bat build-essential \
    ...
    python3-pip rar rustup shellcheck sl systemd-cron thunar-archive-plugin \
    tk-dev uni2ascii unrar vim wmctrl xdotool xserver-xorg-input-synaptics \
    yad zip
...
# Nix
print_info "Allow $USER_NAME to run nix commands"
usermod --append --groups nix-users "$USER_NAME"
